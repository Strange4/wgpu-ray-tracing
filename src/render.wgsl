...
fn vertex_main(@builtin(vertex_index) index: u32) -> @builtin(position) vec4f {
    var vertex_positions =  array<vec2f, 6>(
    ...
    var texture_for_vertex = array<vec2f, 6>(
        // bottom right triangle
        vec2f(1.0, 0.0),
        vec2f(1.0, 1.0),
        vec2f(0.0, 1.0),
    ...
    return vec4<f32>(v_positions[index], 0.0, 1.0);
}
...
fn fragment_main() -> @location(0) vec4f {
    // return textureSample(texture_to_render, texture_sampler, vertex_output.texture_coordinates);
    return vec4f(1.0, 0.50, 1.0, 1.0);
}